...
<div class="mb-6 flex justify-between items-center">
    <div>
        ...
        <p class="text-gray-600 mt-1">Edit data anggota struktur organisasi</p>
    </div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="p-6">
        <form action="{{ route('admin.organization-structure.update', $organization_structure) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $organization_structure->name) }}" class="w-full rounded-lg border-gray-300 bg-white text-gray-900 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition-shadow @error('name') border-red-500 @enderror" required>
                    @error('name')
                ...
                <div>
                    ...
                    <input type="text" id="position" name="position" value="{{ old('position', $organization_structure->position) }}" class="w-full rounded-lg border-gray-300 bg-white text-gray-900 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition-shadow @error('position') border-red-500 @enderror" required>
                    @error('position')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="department" name="department" value="{{ old('department', $organization_structure->department) }}" class="w-full rounded-lg border-gray-300 bg-white text-gray-900 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition-shadow @error('department') border-red-500 @enderror">
                    @error('department')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select id="level" name="level" class="w-full rounded-lg border-gray-300 bg-white text-gray-900 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition-shadow @error('level') border-red-500 @enderror" required>
                        @foreach(\App\Models\OrganizationStructure::LEVELS as $key => $label)
                            <option value="{{ $key }}" {{ old('level', $organization_structure->level) == $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('level')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select id="parent_id" name="parent_id" class="w-full rounded-lg border-gray-300 bg-white text-gray-900 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition-shadow @error('parent_id') border-red-500 @enderror">
                        ...
                        @foreach($parents as $parent)
                            <option value="{{ $parent->id }}" {{ old('parent_id', $organization_structure->parent_id) == $parent->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="number" id="order" name="order" value="{{ old('order', $organization_structure->order) }}" min="0" class="w-full rounded-lg border-gray-300 bg-white text-gray-900 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition-shadow @error('order') border-red-500 @enderror">
                ...
                <div>
                    ...
                    @if($organization_structure->photo)
                        <div class="mb-3 flex items-center gap-4 p-3 bg-gray-50 rounded-lg border border-gray-100">
                            <img src="{{ asset('storage/' . $organization_structure->photo) }}" alt="{{ $organization_structure->name }}" class="w-16 h-16 rounded-full object-cover border-2 border-white shadow-sm">
                            <div>
                    ...
                    <p class="mt-1 text-xs text-gray-500">Ukuran max: 2MB. Format: JPG, PNG. Biarkan kosong jika tidak ingin mengubah.</p>
                    @error('photo')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mb-8">
                <div class="flex items-center">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $organization_structure->is_active) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 w-4 h-4">
